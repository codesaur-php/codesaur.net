...
- **Templates now HTML-escape every `{{ }}` by default (`codesaur/template` ^5.0).** Until now the engine printed values verbatim and XSS protection depended on the author remembering `|e` on each user-supplied print - a single forgotten filter on a title, comment, name or setting was an injection point on the public site and in the dashboard. With the upgrade the engine escapes strings like Twig does, `|e` / `|e('js')` results, `Markup` objects, nested template objects (the `{{ content }}` layout injection), macro results and numbers stay unescaped, and anything else that produced HTML must be marked with `|raw`. The audit of the 124 templates (2913 prints) found the framework already passing the layout content as a template object and using `|raw` on the CMS content fields and web menu titles, so only these adjustments were needed: the six `|e|nl2br` comment/message prints (public news and product pages, the dashboard comment, review and message views) gained `|raw` because `nl2br` returns a plain string whose `<br />` would otherwise be escaped; `comments-view.html` prints the news body with `|raw`; `Web\Template\ExceptionHandler` and `Dashboard\Exception\ErrorHandler` wrap their already-escaped message HTML in `Markup` so the error pages render the wrapper markup once instead of showing it as text; the e-mail bodies built with `MemoryTemplate` (contact message, news comment, product review, order confirmation / notification / status, development request created / responded) no longer call `htmlspecialchars()` before `set()` - the template does it, so the manual call would have double-escaped names and titles - and wrap their `nl2br()` of the multi-line message in `Markup`; the e-mail subject templates call `setAutoEscape(false)` since a subject is plain text (`Tom & Jerry` must not become `Tom &amp; Jerry`). Side effects worth knowing: the customer-facing order confirmation e-mail, which set the customer name and product title without any escaping, is now escaped; values printed inside inline `<script>` string literals without `|e('js')` now carry HTML entities (`'` -> `&#039;`), which is harmless where the string is written with `innerHTML` and safer than the previous raw quote that could terminate the literal. `tests/Unit/Template/AutoescapeTest.php` guards the contract: the default state, the nested-template and `Markup` cases, both error handlers, the e-mail subject/body patterns, and two static scans - every `|nl2br` / `|json_encode` print must end with `|raw`, `|e` or `|e('js')`, and no PHP file may pass `htmlspecialchars(...)` or a bare `nl2br(...)` to `->set()`. CLAUDE.md "Create Templates" documents the rules for new modules.
- **Client-chosen log ordering goes through the model's column whitelist (`codesaur/dataobject` ^10.2).** `LogsController::retrieve()` accepted any `"word ASC|DESC"` string from the request as the ORDER BY clause; the regex blocked injection but not an undeclared column, which surfaced as an SQL error. The clause is now split into column and direction, the column checked with `Logger::hasColumn()` and the fragment built by the new `Logger::orderBy()` (driver-quoted identifier, uppercased direction), falling back to `id DESC` for anything else - so the list is always ordered even when the client sends nothing. `tests/Unit/Log/LogsRetrieveSanitizationTest.php` mirrors the new logic with a real `Logger` on SQLite, including undeclared columns and the exception path of `orderBy()` itself. CLAUDE.md "Parameterized Queries" names `orderBy()` / `assertColumn()` as the way to put user input into an identifier position.
- `composer.json` requires `codesaur/template` `^5.0.0` and `codesaur/dataobject` `^10.2.1`.

---

## [5.3.0] - 2026-09-08
[5.3.0]: https://github.com/codesaur-php/Raptor/compare/v5.2.1...v5.3.0

### Added

- **Public web language is now part of the URL, so search engines can index every language.** The language switch used to be `/session/language/{code}` -> 302 -> session cookie -> `/`, which gave every language the same URL: a crawler never receives the session cookie, so only the default language was ever indexed and the translated content was invisible to search. Now the default language stays unprefixed (`/news/x`) and every other active language lives under its two-letter code (`/en/news/x`). `public_html/index.php` matches `^/([a-z]{2})(?=/|$)` on the path, mounts `Web\Application` on `/{code}` and passes the code as the `language_prefix` request attribute; because the prefix is the application's mount path, `match()` strips it before routing and `|link` / `generateRouteLink()` prepend it to every generated URL, so routers stay prefix-naive and nothing is hardcoded. `LocalizationMiddleware` gained the resolution order prefix -> session -> default, throws a 404 for an inactive code (`/de/...`), and its session key is now nullable: the Web app passes `null`, so the public site never reads or writes a language session (one URL always means one language), while the Dashboard keeps its session-based `RAPTOR_LANGUAGE_CODE`. The Web `SessionMiddleware` closure strips the language prefix before its `/session/` check so `/en/session/contact-send` keeps its writable session. `Web\Template\TemplateController::webTemplate()` computes three new layout variables from the current request: `language_urls` (the current page in every language - the layout's language dropdown now switches language in place instead of redirecting home; on a single-language record page the other languages point to their home, matching the old behaviour), `hreflang_urls` (only for pages that exist in every language: lists, home and `code='*'` records - a single-language record announces no alternates, so no missing translation is claimed) and `canonical_url` (self URL; a `code='*'` record is canonicalised to the default-language URL because its content is identical under every prefix). `index.html` emits `<link rel="canonical">` plus the `hreflang` set including `x-default`. `SeoController::sitemapXml()` now selects each record's `code` and lists it under its own language prefix (`*` records under every language) plus one home URL per language, and `rss()` links carry the current prefix. `/session/language/{code}` is kept for old links but only redirects to that language's home. Rule for projects: never register a web route whose first segment is exactly two lowercase letters, and page `link` fields holding a local path are not prefixed (use a slug or an absolute URL). Deploying to an existing site needs no configuration or data change: old unprefixed URLs keep serving the default language, and visitors who had switched language via the session simply see the default language until they pick one again (no `.env` variable - the behaviour is unconditional).
- **Language-neutral News, Pages and Products: a record with `code = '*'` shows on every language of the public site.** Until now a news item, page or product existed in exactly one language, so content that is the same in every language (a PDF notice, a photo gallery, a bilingual announcement, a product with a language-independent name) had to be duplicated per language and kept in sync by hand. The dashboard language dropdown on the insert/update forms of all three modules now ends with an "All languages" entry (new `all-languages` text keyword) that stores the sentinel `'*'` in the existing `code` column - the Joomla `language='*'` / TYPO3 "All languages" convention, so no schema change and no data migration. Every web-side language filter switched from `code=:code` to `code IN (:code, '*')`: news type lists and archive, navigation menu and featured pages (`PagesModel`), page siblings, product list, HTML sitemap, RSS feed, site search and the home page's recent news (`NewsModel`). `Web\Template\TemplateController::webTemplate()` no longer maps `code='*'` into `record_code`, so a language-neutral page renders `<html lang="mn">` (the current site language) instead of the invalid `lang="*"`. Wherever the dashboard renders a flag from `code` (insert/update/view forms, the news/pages/products index tables, the pages navigation tree - which gains a separate "All languages" card listed last - and the index language filters), `'*'` is shown as a globe icon with the "All languages" label instead of a broken flagcdn image. Pages tree rule: a parent page must share the child's language or itself be "All languages" (`PagesController::isParentCodeCompatible()`, mirrored client-side by `filterParentsByCode()` in `page-insert.html`; the update form's parent list includes `*` parents) - a language-neutral child under a single-language parent would be orphaned in the other languages' navigation. The same rule is now also enforced downward on update: changing a page's language while it has direct children of an incompatible language (e.g. an "All languages" parent with `mn` children switched to `en`) is rejected with the new `change-child-pages-language-first` message instead of saving and silently dropping the children from their language's navigation (`PagesModel::getNavigation()` skips nodes whose parent is absent from the language's page set). This closes a gap that predates the sentinel - an `mn` parent with `mn` children could already be switched to `en` with the same silent effect. Only direct children are checked, since grandchildren are already compatible with their own parent. Cycles remain impossible through language edits: the tree recursion runs on `parent_id` only, guarded by the existing descendant check and `findAncestry()`. The rule set is documented in CLAUDE.md ("Language-neutral Records") and guarded by `tests/Unit/Web/LanguageNeutralRecordsTest.php`, which fails if any web-side query drops back to a bare `code=:code`. Module manuals (news/pages/products, MN + EN) describe the option. Already-deployed databases need the two new translation keywords: `INSERT INTO localization_text (keyword, type, created_at) VALUES ('all-languages', 'sys-defined', NOW()), ('change-child-pages-language-first', 'sys-defined', NOW()); INSERT INTO localization_text_content (parent_id, code, text) SELECT id, 'mn', 'Бүх хэл' FROM localization_text WHERE keyword = 'all-languages'; INSERT INTO localization_text_content (parent_id, code, text) SELECT id, 'en', 'All languages' FROM localization_text WHERE keyword = 'all-languages'; INSERT INTO localization_text_content (parent_id, code, text) SELECT id, 'mn', 'Дэд хуудсуудын хэл шинэ хэлтэй нийцэхгүй байна - эхлээд дэд хуудсуудын хэлийг солино уу' FROM localization_text WHERE keyword = 'change-child-pages-language-first'; INSERT INTO localization_text_content (parent_id, code, text) SELECT id, 'en', 'Child pages do not match the new language - change the child pages first' FROM localization_text WHERE keyword = 'change-child-pages-language-first';` (fresh installs get them from `TextInitial.php`; without the rows the UI shows the bare keywords, nothing breaks). `LocalizedModel`-based modules (Text, Settings, Reference, Menu) are unaffected - they store one content row per language and have no `*` concept.
