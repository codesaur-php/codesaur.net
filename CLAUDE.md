...
- The settings fields `urgent`, `contact`, `address` and `copyright` are documented to admins as "supports HTML" (trusted admin content), and `dashboardTemplate()` / `webTemplate()` hand them to templates as bare variables - so every bare print of them ends with `|raw` (`{{ address|raw }}`), otherwise the admin's `<p>` / `<br>` / `<a>` shows as source text. The same test scans for it.
...
**Return to the intended page after login.** When an anonymous visitor hits a dashboard route, `JWTAuthMiddleware::loginRedirectTarget()` decides whether to record where they were going, and the login URL becomes `/dashboard/login?redirect=<full path + query>`. A target is recorded ONLY for browser page loads - GET/HEAD with `Accept: text/html` - so a `fetch()`ed JSON or fragment route that hits an expired session never becomes the post-login landing page; the dashboard root and `/dashboard/home` are skipped too since they are the default destination. `LoginController::sanitizeRedirectTarget()` (static, so it is directly unit-testable) validates the value and `renderLogin()` hands it to `login.html` as `redirect_url`, which the form carries in `data-redirect` and the success handler prefers over the `home` route; `index()` honors the same target when an already-authenticated visitor opens the login page. That sanitizer is the ONLY thing standing between the query parameter and an open redirect, so keep all of its rules: non-empty string starting with `/` but not `//`, no control characters and no backslashes (CR/LF header injection, `/vil.com` tricks), must sit under `getScriptPath() . getMountPath()`, never a login path (redirect loop). Covered by `tests/Unit/Authentication/LoginRedirectTargetTest.php` - extend those tests rather than loosening the check. The middleware also clears the login URI's query and fragment before adding `redirect`; without that, `withPath()` would carry the blocked URL's own query string over and let a stray `?forgot=` or `?signup=` drive the login page.
